...
pub async fn init_db() -> Result<DatabaseConnection> {
    ...
    let sql = backend.build(&stmt).to_string().replace("CREATE TABLE \"users\"", "CREATE TABLE IF NOT EXISTS \"users\"");
    db.execute_unprepared(&sql).await?;
    
    let stmt = schema.create_table_from_entity(crate::client::Entity);
    let sql = backend.build(&stmt).to_string().replace("CREATE TABLE \"oauth_clients\"", "CREATE TABLE IF NOT EXISTS \"oauth_clients\"");
    db.execute_unprepared(&sql).await?;
    
    let stmt = schema.create_table_from_entity(crate::auth_code::Entity);
    let sql = backend.build(&stmt).to_string().replace("CREATE TABLE \"auth_codes\"", "CREATE TABLE IF NOT EXISTS \"auth_codes\"");
    db.execute_unprepared(&sql).await?;
    
    let stmt = schema.create_table_from_entity(crate::access_token::Entity);
    let sql = backend.build(&stmt).to_string().replace("CREATE TABLE \"access_tokens\"", "CREATE TABLE IF NOT EXISTS \"access_tokens\"");
    db.execute_unprepared(&sql).await?;

    let stmt = schema.create_table_from_entity(crate::refresh_token::Entity);
    let sql = backend.build(&stmt).to_string().replace("CREATE TABLE \"refresh_tokens\"", "CREATE TABLE IF NOT EXISTS \"refresh_tokens\"");
    db.execute_unprepared(&sql).await?;
...
pub async fn create_clients(db: &DatabaseConnection) -> Result<()> {
    let clients = vec![
        ...
        (
            ...
            "test-secret", 
            "Local Test Client",
            vec!["http://localhost:3000/auth/callback", "http://localhost:3001/auth/callback"],
