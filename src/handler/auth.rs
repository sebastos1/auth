...
use crate::{
    errors::{AuthError, error_page, error_redirect},
...
pub async fn get(
    ...
    let client = crate::client::Entity::find_by_id(&params.client_id)
        .one(&db)
        .await
        .map_err(|_| {
    ...
    let allowed_scopes = client.get_allowed_scopes().map_err(|_| {
        let error_page = error_page(AuthError::ServerError).unwrap();
        (StatusCode::INTERNAL_SERVER_ERROR, error_page)
    })?;
...
pub async fn post(
    ...
    let valid = verify(&form.password, &user.password_hash).map_err(|_| {
        error_redirect(
            AuthError::ServerError,
            &form.redirect_uri,
            form.state.as_deref(),
    ...
    auth_code.insert(&db).await.map_err(|_| {
        error_redirect(
            AuthError::ServerError,
            &form.redirect_uri,
            form.state.as_deref(),
    ...
    let mut redirect_url = url::Url::parse(&form.redirect_uri).map_err(|_| {
        error_redirect(
